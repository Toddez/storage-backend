...
const identify = (ext) => {
  for (const type of Object.keys(Extensions)) {
    for (const ending of Extensions[type]) {
      if (ending === ext) return parseInt(type);
    }
  }
...
class Node {
  constructor(type, parent, localPath) {
    ...
    if (this.type & NodeType.ROOT) this.data.resolvedPath = "root";
...
class Storage {
  ...
  crawl(node) {
    if (node.type & NodeType.CRAWLABLE) {
      ...
      for (const file of files) {
        ...
        if (stat.isDirectory()) type = NodeType.DIR;

        const fileName = this.decrypt(file).toString("utf8");
        const extension = getExtension(fileName);

        if (stat.isFile()) type = identify(extension);
  ...
  find(node, localPath, depth = 0) {
    for (const child of node.children) {
      ...
      if (current === compare) return this.find(child, localPath, depth + 1);
    }
  ...
  createDir(localPath) {
    ...
    const resolvedPath = [
      deepest.localPath,
      ...spl.slice(depth, spl.length).map((bit) => this.encrypt(bit)),
    ].join("/");
  ...
  writeFile(localPath, data) {
    const spl = localPath.split("/");
    if (spl.length > 1) {
      if (!this.createDir(spl.slice(0, -1).join("/"))) return false;
  ...
  readFile(localPath) {
    ...
    if ("root/" + localPath !== deepest.data.resolvedPath) return "";

    try {
      return this.decrypt(
        fs.readFileSync(this.path(deepest.localPath)).toString("utf8")
      );
    } catch (err) {
      return "";
    }
  ...
  delete(localPath) {
    ...
    if ("root/" + localPath !== deepest.data.resolvedPath) return false;

    const fullPath = this.path(deepest.localPath);

    try {
      if (deepest.type & NodeType.DIR) deleteFolderRecursive(fullPath);
      else if (deepest.type & NodeType.FILE) fs.unlinkSync(fullPath);
  ...
  rename(localPath, name) {
    ...
    if ("root/" + localPath !== deepestPrevious.data.resolvedPath) return false;
    ...
    const resolvedPath = this.path(
      [
        deepestTarget.localPath,
        ...spl.slice(depth, spl.length).map((bit) => this.encrypt(bit)),
      ].join("/")
    );
