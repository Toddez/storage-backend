  ...
  return new Promise((resolve) => {
    bcrypt.hash(key, saltrounds, (err, hash) => {
      Database.collection("users").insertOne(
        { id: sha256(id), key: hash, auth: false, authKey: null },
        { new: true },
        (err, response) => {
...
router.post("/2faqrc", async (req, res, next) => {
  ...
  jwt.verify(token, jwtSecret, async (err, payload) => {
    ...
    if (
      await Database.collection("users").findOne({ id: sha256(payload.id) })
        .auth
    ) {
      let err = new Error("Invalid token");
      err.status = 403;
...
router.post("/authorize", (req, res, next) => {
  ...
  Database.collection("users").findOne(
    ...
    async (err, user) => {
      ...
      let newUser = !user;
      if (newUser) user = await createUser(id, key);
      ...
      bcrypt.compare(key, user.key, (_, result) => {
        if (result) {
          const payload = {
            id: id,
            key: key,
            first: newUser || user.authKey === null || user.auth === false,
            auth: false,
          };
